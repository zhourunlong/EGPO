...
class ExtragradientTrainer(OnlineDPOTrainer):
    ...
    def training_step(
        ...
        if self.args.estimate_extra_grad:
            if self.state.global_step % 2 == 1:
                gpu_optimizer_state_dict = move_state_dict_to_device(self.optimizer_backup, next(model.parameters()).device)
    ...
    def create_model_card(
        ...
        model_card = generate_model_card(
            ...
            paper_title="Extragradient Preference Optimization (EGPO): Beyond Last-Iterate Convergence for Nash Learning from Human Feedback",
            paper_id="2503.08942",
        )
