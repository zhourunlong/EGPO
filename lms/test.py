...
test_dataset = transform_into_chat(dataset["train"], max_data_num=-NUM_DATA)
...
# ]
# BASE_NAMES = [
#     "vectorzhou/gemma-2-2b-it-alpaca-cleaned-SFT-PKU-SafeRLHF-OnlineIPO1-0317153039",
#     "vectorzhou/gemma-2-2b-it-alpaca-cleaned-SFT-PKU-SafeRLHF-OnlineIPO2-0320211428",
#     "vectorzhou/gemma-2-2b-it-alpaca-cleaned-SFT-PKU-SafeRLHF-NashMD-0323231936",
#     "vectorzhou/gemma-2-2b-it-alpaca-cleaned-SFT-PKU-SafeRLHF-NashMDPG-0401225210",
#     "/local1/vectorzh/experiments/nlhf/MPO-Safe/MPO",
#     "vectorzhou/gemma-2-2b-it-alpaca-cleaned-SFT-PKU-SafeRLHF-Extragradient-0504174751"
# ]

BASE_NAMES = [
    "vectorzhou/vectorzhou-Qwen2-5-1-5B-Instruct-SFT-OpenHerm-ction-v0-1-OnlineIPO1-lora-0603201242",
    "vectorzhou/vectorzhou-Qwen2-5-1-5B-Instruct-SFT-OpenHerm-ction-v0-1-OnlineIPO2-lora-0604063354",
    "vectorzhou/vectorzhou-Qwen2-5-1-5B-Instruct-SFT-OpenHerm-ollection-v0-1-NashMD-lora-0603201151",
    "vectorzhou/Qwen2.5-1.5B-Instruct-SFT-OpenHerm-llection-v0.1-NashMDPG-lora-0605154608",
    "vectorzhou/vectorzhou-Qwen2-5-1-5B-Instruct-SFT-OpenHerm-on-v0-1-Extragradient-lora-0604122854",
]
...
if __name__ == "__main__":
    ...
    for base_name in BASE_NAMES:
        if PICK_TYPE == "last":
            for k in range(NUM_CKPT):
                all_models.append(f"{base_name}-epoch-{NUM_EPOCHS-k}")
        elif PICK_TYPE == "top":
            win_rates = {}
            for epoch in range(NUM_EPOCHS):
                model1 = f"{base_name}-epoch-{epoch+1}"
                win_rates[epoch+1] = get_pref(compare_results, model1, REF_MODEL, JUDGE_TYPE)
            sorted_win_rates = sorted(win_rates.items(), key=lambda x: x[1], reverse=True)

            for k in range(NUM_CKPT):
                all_models.append(f"{base_name}-epoch-{sorted_win_rates[k][0]}")
        else:
            raise NotImplementedError
